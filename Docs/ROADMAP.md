...
| **P** | **Class-Tools System** (classes bring different tools to solve different problems) | 🟡 4/6 — **design ✅** (`Class-Tools-System.md`); **P.0** done (boss-dive fix → C.10/C.8 reads were survival artifacts); **P.1** done (school tax net +4; Bard→Archer recut); **P.2** done (real cast scheduler + interrupt — Bellreach kick-or-wipe; +3-4 kick gap); **P.3 done** (dispel typing — Mire P4: a stacking **Nature** curse only the Lifebinder's Nature dispel resets → Lifebinder tops the Mire **+5** over a Cleric who can't strip it; rot softened to a +1 spice so the curse carries the read); next **P.4 reach/adds/avoidable-immunity**. generic-EHP + spec-power + DPS-check → P.5 |
...
## Phase P — Class-Tools System (classes bring different tools to solve different problems) 🟡 (4/6)
