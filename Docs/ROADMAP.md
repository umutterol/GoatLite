...
| **P** | **Class-Tools System** (classes bring different tools to solve different problems) | 🟡 3/6 — **design ✅** (`Class-Tools-System.md`); **P.0** done (boss-dive fix → C.10/C.8 reads were survival artifacts); **P.1** done (school tax net +4; Bard→Archer recut); **P.2 done** (real cast scheduler + interrupt — Bellreach kick-or-wipe: kicker interrupts 24/25 casts → times, no-kicker wipes; +3-4 kick gap); next **P.3 dispel typing** (Mire P4). generic-EHP + spec-power + DPS-check → P.5 |
