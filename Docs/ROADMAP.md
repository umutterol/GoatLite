...
| P.5f | **Magic-spec pass — pyromancer single-target buff** | balance/content | minor | S | ⬜ | **Spun out of P.5c-M3/M4 (2026-06-23).** The timer-bound regime made HP-sponge bosses the DPS-check, exposing **magic≪physical on bosses** (pyro/arcanist AoE-flavored). M3 fixed the broken-low arcanist; **pyromancer single-target is still weak (mono +7 boss vs assassin +22)**, which (a) undermines the **Pyreward "bring magic" school read** (mitigated physical still out-clears magic → the wall can't make magic win — softening K inverts it) and (b) leaves magic comp-viable but not boss-competitive. Buff pyro ST casts → finish magic parity + restore the Pyreward off-school tax (target +3–5). Touches egm-smoke (pyro in the comp) → re-verify the regime holds. |
