...
- **2026-06-21** — **Replay arena: combatants now CLOSE on each other (EGM-style movement, faked).** Following up the
  v2 arena, the dots no longer hold separate columns and shoot across a static gap — they engage. Added a movement
  pass to `ReplayCanvas.tsx` (UI-only, still no sim positions): an **engaged formation** (`eng` ramps over a stage's
  first seconds = the charge-in) sends **melee to a center contact line**, where party-melee **pair off against living
  foes and re-seek** as those foes die (so the scrum relocates to survivors), while **ranged hold back** and fire
  projectiles across; on top sit a melee **press-sway** and a **per-attack lunge** (melee jab toward their target on
  each hit, via a `transform` translate eased by the `.replay-dot` transition). Verified: `tsc -b` clean; live check
  (`scripts/i-live.mjs`) 9/9, 0 console errors; play-frame screenshots show the melee charging into the pack,
  intermingling with enemy tokens, and relocating to the last survivor as mobs die. No engine change → determinism
