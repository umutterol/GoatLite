...
| **P** | **Class-Tools System** (classes bring different tools to solve different problems) | 🟡 5/6 — **design ✅** (`Class-Tools-System.md`); **P.0–P.3 done** (boss-dive fix; school tax +4 + Bard→Archer; cast scheduler+interrupt = Bellreach kick-or-wipe; dispel typing = Mire P4 Nature-curse, Lifebinder +5 over Cleric); **P.4 done** (user recut P11 reach → a **shield/guard kill-priority** mechanic realized as a **summon-shield boss** unifying P8 real adds: the Leaden Carillon summons a `guarding` warden that wards it 99% until cut down — Hour spread 2, fair ~1–2-key cost, other dungeons untouched; **P7 eruption deferred**); next **P.5 anti-heal/purge + self-healing enemy + final kit recut + generic-EHP kill + spec-power/DPS-check** (the big balance milestone — also owns the P.4 "flat-damper→sharp-read" + magic≫physical fixes) |
...
| C.2 | **Per-spec talent trees** (now the prioritized pre-P.5c work) | content | major | XL | 🟡 | **Design in progress (2026-06-22).** Re-scoped: **≥15 nodes/spec grouped into ~5 tiers of 3 mutually-exclusive choices** (pick 1 of 3 per tier), each a **real playstyle tradeoff** (AoE↔single-target · survival/defensive↔output · self-sustain↔raw damage · burst↔sustained · lean-into↔away-from the spec's class-tool), **WoW-inspired** (retail + classic talent design) but with **earnest in-world gothic names** (no meta/QA/dev puns — match Drag-to-the-Dust / Barrow-Step / Unbinding Word / Creeping Rot). Covers **all 12 specs** (the 10 current + the planned Necromancer's Bonecaller/Pallbinder). Design report: `Docs/Talent-Trait-Revamp.md` (multi-agent investigation). **Sequenced BEFORE P.5c** (talents are sim power inputs — calibrating the regime before the revamp = double-tuning; see P.5/Open-Decisions). Then engine-wire the new effect vocab the design needs |
